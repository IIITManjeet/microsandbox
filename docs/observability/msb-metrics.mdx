...
`memory.usage` can briefly be off until the guest's memory statistics catch
up. Sandboxes started by an older runtime keep reporting their boot allocation

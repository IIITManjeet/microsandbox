...
| `tls` | Restart or next start | Configuring a TLS-identity secret can turn interception on, and interception cannot start on a running sandbox |
...
Configuring a secret that requires TLS identity on a sandbox with interception off also turns it on. That shows up in the plan as a `tls` change and needs a restart or the next start. Existing secrets configured with `require_tls_identity(false)` continue to use plain-HTTP substitution without enabling interception. Removing every TLS-dependent secret leaves interception on.

...
Wait until every live resize is terminal. Returns an empty slice when the sandbox is not running. The untimed variant keeps polling while a guest never converges; the timed variant returns a `*ResizeTimeoutError` on expiry, with the last observed status in its `Status` field.
